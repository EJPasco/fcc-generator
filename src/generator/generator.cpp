...
int main(int argc, char * argv[]){
	...
	while(keyptc_counter < nevents) {
		if(pythia.next()) {
			...
			std::size_t keyptcs_in_event = 0; // number of "key" particles in a particlar event

			// counting "key" particles in the event
			for(auto ip = hepmcevt->particles_begin(), endp = hepmcevt->particles_end(); ip != endp; ++ip) {
				if(abs((*ip)->pdg_id()) == keyptc && isBAtProduction(*ip)) {
					++keyptcs_in_event;
				}
			}

			keyptc_counter += keyptcs_in_event;

			if(keyptcs_in_event > 0) {
				if(verbosity >= 2) {
	...
	std::cout << "Elapsed time: " << elapsed_time << " s. Mean rate: " << keyptc_counter / elapsed_time << " ev / s." << std::endl;
